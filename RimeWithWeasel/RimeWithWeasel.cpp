...
DWORD RimeWithWeaselHandler::RemoveSession(WeaselSessionId ipc_id) {
  ...
  m_active_session = 0;
  if (m_last_input_session == ipc_id)
    m_last_input_session = 0;
  return 0;
}
...
weasel::QuickSwitchSnapshot RimeWithWeaselHandler::GetQuickSwitches(
  ...
  for (const auto& source :
       weasel::LoadSwitchGroups(rime_api, snapshot.schema_id)) {
    weasel::QuickSwitchGroup group;
    group.schema_index = source.index;
    group.options = source.options;
    group.label = u8tow(weasel::SwitchDisplayName(
        source, PRIMARYLANGID(GetThreadUILanguage()) != LANG_CHINESE));
    const size_t count = weasel::SwitchStateCount(source);
    for (size_t i = 0; i < count; ++i) {
      const std::string label =
          i < source.states.size() && !source.states[i].empty()
              ? source.states[i]
              : (source.options.size() > 1 ? source.options[i]
                                           : (i ? "On" : "Off"));
      group.states.push_back(u8tow(label));
    }
    if (source.options.size() == 1) {
      group.selected = rime_api->get_option(session->second.session_id,
                                            source.options.front().c_str())
                           ? 1
                           : 0;
    } else {
      for (size_t i = 0; i < source.options.size(); ++i) {
        if (rime_api->get_option(session->second.session_id,
                                 source.options[i].c_str())) {
          group.selected = static_cast<int>(i);
          break;
        }
      }
    }
...
bool RimeWithWeaselHandler::SelectQuickSwitch(
    const weasel::QuickSwitchSnapshot& snapshot,
    int schema_index,
    int state) {
  if (m_disabled || !snapshot.session_id || snapshot.schema_id.empty())
    return false;
  const auto session = m_session_status_map.find(snapshot.session_id);
  if (session == m_session_status_map.end() || !session->second.session_id ||
      !rime_api->find_session(session->second.session_id))
    return false;
  RIME_STRUCT(RimeStatus, status);
  ...
  const bool same_schema =
      status.schema_id && snapshot.schema_id == status.schema_id;
  rime_api->free_status(&status);
  if (!same_schema)
    return false;
  const auto groups = weasel::LoadSwitchGroups(rime_api, snapshot.schema_id);
  const auto found = std::find_if(groups.begin(), groups.end(),
                                  [schema_index](const auto& group) {
                                    return group.index == schema_index;
                                  });
  if (found == groups.end() || state < 0 ||
      static_cast<size_t>(state) >= weasel::SwitchStateCount(*found))
    return false;
  if (found->options.size() == 1) {
    rime_api->set_option(session->second.session_id,
                         found->options.front().c_str(), state != 0);
  } else {
    for (size_t i = 0; i < found->options.size(); ++i)
      rime_api->set_option(session->second.session_id,
                           found->options[i].c_str(),
                           i == static_cast<size_t>(state));
  }
  _UpdateUI(snapshot.session_id);
  return true;
}
