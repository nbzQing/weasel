...
// class ServerImpl
{
 public:
  ...
  DWORD OnGetQuickSwitches(WEASEL_IPC_COMMAND uMsg, DWORD wParam, DWORD lParam);
  DWORD OnSelectQuickSwitch(WEASEL_IPC_COMMAND uMsg,
                            DWORD wParam,
                            DWORD lParam);
 ...
 public:
  ...
  void SetDynamicMenuHandler(std::function<bool(UINT)> handler) {
    m_dynamicMenuHandler = std::move(handler);
  }
 ...
 private:
  ...
  std::function<bool(UINT)> m_dynamicMenuHandler;
