...
WeaselPanel::WeaselPanel(weasel::UI& ui)
    : m_layout(NULL),
      ...
      m_inputPos(CRect()),
      m_lastCaretPos(CRect()),
      m_hasLastCaretPos(false),
      m_sticky(false),
      dpi(96),
...
void WeaselPanel::MoveTo(RECT const& rc) {
  ...
  // _RepositionWindow() reuses m_inputPos.bottom for the adjusted window Y.
  // Compare host caret rectangles instead, or an above-caret placement would
  // look like a large caret movement on every subsequent keystroke.
  const CRect previousCaretPos = m_lastCaretPos;
  const bool hadPreviousCaretPos = m_hasLastCaretPos;
  bool should_reset_sticky = (m_ctx.empty() || !hadPreviousCaretPos ||
                              (abs(rc.left - previousCaretPos.left) > 50) ||
                              (abs(rc.bottom - previousCaretPos.bottom) > 50));
  if (should_reset_sticky && m_sticky) {
    m_sticky = false;
    // Force reposition the window
    m_lastCaretPos = rc;
    m_hasLastCaretPos = true;
    m_inputPos = rc;
  ...
  if (m_style.ascii_tip_follow_cursor && m_ctx.empty() &&
    ...
    _RepositionWindow(true);
    RedrawWindow();
  } else if (!(hadPreviousCaretPos && rc.left == previousCaretPos.left &&
               rc.top == previousCaretPos.top &&
               rc.bottom != previousCaretPos.bottom &&
               abs(rc.bottom - previousCaretPos.bottom) < 6) ||
